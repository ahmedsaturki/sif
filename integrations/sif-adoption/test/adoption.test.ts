...
test("ADOPT-005 rejects a reused requestId with changed input", async () => {
  ...
  await assert.rejects(
    () => gateway.execute(envelope({ payload: { changed: true } })),
    (error: unknown) =>
...
test("ADOPT-016 preserves deterministic envelope digests", async () => {
  ...
  const b = await gateway.execute(envelope());
...
test("ADOPT-020 gateway exposes deterministic default limits", () => {
  const limits = defaultSifAdoptionLimits();
  assert.equal(limits.maxEnvelopeBytes, 192 * 1024);
  assert.equal(limits.maxProcessedRequests, 4096);
});

test("ADOPT-021 concurrent duplicate requests execute the handler once", async () => {
  let calls = 0;
  const gateway = createDefaultSifAdoptionGateway({
    ...handlers(),
    "policy.check": async ({ request }) => {
      calls += 1;
...
test("ADOPT-022 Core response tampering remains detectable", async () => {
  const gateway = createDefaultSifAdoptionGateway(handlers());
  const result = await gateway.execute(envelope());
  const tampered: SifProductResponse = { ...result.response, output: { tampered: true } };
  assert.throws(
    () => verifySifProductReplay(envelope().request, tampered, defaultSifAdoptionLimits()),
  );
});
