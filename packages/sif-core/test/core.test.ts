...
test("filesystem CAS writes and verifies immutable content", async () => { const { FileCas } = await import("../src/cas.js"); const { mkdirSync, rmSync } = await import("node:fs"); const root = `.ci/sif-cas-${Date.now()}-${Math.random().toString(16).slice(2)}`; mkdirSync(root, { recursive: true }); try { const cas = new FileCas(root); const bytes = new TextEncoder().encode("sif-cas-test"); const digest = cas.putBytes(bytes); assert.equal(cas.has(digest), true); assert.deepEqual(cas.getBytes(digest), bytes); assert.doesNotThrow(() => cas.verify(digest)); } finally { rmSync(root, { recursive: true, force: true }); } });