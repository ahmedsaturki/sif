...
test("F5-060",async()=>{const s=new InMemoryObservationSink();const z=await new RegressionSuiteRunner(s,L).run("final",C,[{id:"final",run:async()=>{}}],tr());assert.equal(z.results[0]!.candidateCommit,C);assertPromotionEvidence({candidateCommit:C,artifactDigest:"a",evaluations:z.results,faults:[]});});
