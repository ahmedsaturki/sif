...
const add=(id:string,fn:()=>void)=>cases.push([id,fn]);
...
// Compatibility: F6-016..030
add("F6-016",()=>{const r=cr(),e=new SemanticCompatibilityEngine(r,L);assert.equal(e.resolve({sourceOntologyId:"sif",sourceVersion:"1.0",sourceConceptId:"a",targetOntologyId:"sif",targetVersion:"1.0",targetConceptId:"a"}).status,"EXACT")});
...
add("F6-020",()=>{const e=new SemanticCompatibilityEngine(cr(),L);assert.equal(e.resolve({sourceOntologyId:"sif",sourceVersion:"1.0",sourceConceptId:"a",targetOntologyId:"sif",targetVersion:"2.0",targetConceptId:"b"}).status,"INCOMPATIBLE")});
add("F6-021",()=>{const r=mapReg("equivalent");r.register(base({version:"3"}));const x=r.get("sif","1")!;x.mappings.push({...x.mappings[0]!,mappingId:"m2"});r.register(x);});
...
add("F6-025",()=>{const x=new SemanticCompatibilityEngine(cr(),L).resolve({sourceOntologyId:"sif",sourceVersion:"1.0",sourceConceptId:"a",targetOntologyId:"sif",targetVersion:"1.0",targetConceptId:"a"});assert.equal(x.decisionDigest.length,64)});
